...
export function ImageUploader({ name, label = "تصویر محصول" }: ImageUploaderProps) {
  ...
  // پاک‌سازی آدرس URL بعد از خروج از صفحه
  useEffect(() => {
    return () => {
      if (typeof file !== "string" && file) URL.revokeObjectURL(file);
    };
  }, [file]);
        ...
        {preview ? (
          ...
          <div className="flex flex-col items-center gap-2 text-gray-500">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" fill="#00B4D8" viewBox="0 0 24 24">
              <path d="M12 16v-8m0 0l-4 4m4-4l4 4m5 4v5H3v-5" stroke="#00B4D8" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
